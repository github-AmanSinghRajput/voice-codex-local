...
export function getVoiceSubline(
  ...
  if (state === 'listening') {
    return 'Speak naturally. I will start working as soon as you pause.';
  }

  if (state === 'thinking') {
    return 'Reading context, planning the answer, and preparing the next response.';
  }

  if (state === 'speaking') {
    return 'Replying out loud now. You can interrupt at any time.';
  }

  if (state === 'idle') {
    return 'Start voice chat when you want to talk through code, fixes, or ideas.';
  }
...
export function getSuggestedScreen(
  ...
  const assistantReady = status?.assistantProviders.activeProvider?.appConnected ?? false;
...
export function summarizeApproval(pendingApproval: PendingApproval | null) {
  ...
  const taskCount = pendingApproval.tasks?.length ?? 0;
  return taskCount > 0
    ? `${taskCount} task${taskCount === 1 ? '' : 's'} queued for approval`
    : 'Write request pending approval';
}
